...
export class InfluenceZoneDescription extends DropAreaDescription {
  ...
  getExtraCss(location: Location) {
    const kingdom = location.id as Kingdom
    return css`
      background: ${playerColorCode[kingdom]};
      background: no-repeat center 1em / 30% url(${this.icons[kingdom]}), linear-gradient(180deg, ${playerColorCode[kingdom]}70 0%, ${playerColorCode[kingdom]}00 100%);
    `
  }

  help = InfluenceZoneHelp

  icons: Partial<Record<Kingdom, string>> = {
