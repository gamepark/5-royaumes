...
class FiveKingdomHistoryDescription implements LogDescription<MaterialMove, Kingdom, MaterialGame> {
  getMovePlayedLogDescription(
    move: MaterialMove,
    context: MoveComponentContext<MaterialMove, Kingdom, MaterialGame>
    ...
    if (game.rule?.id === RuleId.DrawBannerCard
      ...
      return { Component: () => <PlaceBannerCardHistory move={move} context={context}/> }
    }
    ...
    if (isMoveItemType(MaterialType.CharacterCard)(move)
      ...
      return { Component: () => <DestroyCardHistory move={move} context={context}/> }
    }

    if (isCustomMoveType(CustomMoveType.ActivateCharacter)(move)) {
      return { Component: () => <ActivateCardHistory move={move} context={context}/> }
    }
    ...
    if (
      ...
      return { Component: () => <TakeBannerCardsHistory move={move} context={context}/> }
    }
    ...
    if (
      ...
      return { Component: () => <InfluenceHistory context={context}/> }
    }
    ...
    if (context.consequenceIndex === undefined && isMoveItemType(MaterialType.CharacterCard)(move) && move.location?.type === LocationType.Discard && game.items[move.itemType]![move.itemIndex].location?.type !== LocationType.PlayerHand) {
      return { Component: () => <DiscardCardHistory move={move} context={context}/> }
    }
    ...
    if (isMoveItemType(MaterialType.CharacterCard)(move) && (move.location?.type === LocationType.Council || move.location?.type === LocationType.PlayerTitan)) {
      return { Component: () => <RecruitCardHistory move={move} context={context}/> }
    }

    if (isCreateItemType(MaterialType.Castle)(move)) {
      return { Component: () => <WinCastleHistory move={move} context={context}/> }
    }

    if (isMoveItemType(MaterialType.Castle)(move)) {
      return { Component: () => <StealCastleHistory move={move} context={context}/> }
    }

    if (isStartRule(move) && move.id === RuleId.EndGame) {
      return { Component: () => <EndGameHistory/> }
    }

    if (isStartRule(move) && move.id === RuleId.RefillAlkane) {
      return { Component: () => <RefillAlkaneHistory context={context}/> }
    }
...
export const FiveKingdomHistory = new FiveKingdomHistoryDescription()
